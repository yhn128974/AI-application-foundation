...
print(df.head(3))
print(df.tail(2))
# 对数值型列统计计数、均值、标准差、四分位数等
print(df.describe())
# 对DataFrame的摘要信息，包括索引类型、列名、非空值数量、内存占用
print(df.info())
# 
print(df['姓名'])
# DataFrame的一个单列类型series
print("姓名列的类型为：", type(df['姓名']))  # <class 'pandas.core.series.Series'>

print(df[['姓名','年龄']])
# DataFrame的一个多列类型DataFrame
print("姓名加年龄列的类型为：", type(df[['姓名','年龄']]))  # <class 'pandas.core.frame.DataFrame'>

# 从行来选择 df.loc[] —— 基于索引标签（行名/列名）进行选择，包含结束位置（左闭右闭）
print(df.loc[['s1']])
print(df.loc[['s2','s2']])
...
print(df.loc['s1':'s3'])      # 选择行's1'和's3'，所有列
print(df.loc['s1':'s3', ['姓名','年龄']])      # 选择行's1'和's3'，所有列的姓名和年龄
